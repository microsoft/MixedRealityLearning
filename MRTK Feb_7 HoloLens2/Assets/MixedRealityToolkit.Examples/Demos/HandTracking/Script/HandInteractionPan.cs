...
public class HandInteractionPan : BaseFocusHandler, IMixedRealityHandTrackHandler, IMixedRealityInputHandler
{
    ...
    private bool TryGetHandRayPoint(IMixedRealityController controller, out Vector3 handRayPoint)
    {
        if (controller != null &&
            ...
            controller.InputSource.Pointers.Length > 0)
        {
            if (controller.InputSource.Pointers[0].Result.RayStepIndex >= 3 || touchType != TouchType.HandRay)
            {
                Ray handRay;
                if (true == controller.InputSource.Pointers[0].TryGetPointingRay(out handRay))
                {
                    Vector3 rayTouchPoint = Vector3.zero;
                    if (true == FindRayCollision(handRay, out rayTouchPoint))
                    {
                        handRayPoint = rayTouchPoint;
                        return true;
                    }
                }
            }
