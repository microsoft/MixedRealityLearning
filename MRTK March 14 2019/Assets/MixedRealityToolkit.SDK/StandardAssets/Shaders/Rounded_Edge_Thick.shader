...
SubShader {
    ...
    //BLOCK_BEGIN Iridescence_Vertex 40

    void Iridescence_Vertex_B40(
        float Intensity,
        sampler2D Texture,
        float Left_X0,
        float Left_X1,
    ...
    //BLOCK_BEGIN Round_Rect_Vertex 33

    void Round_Rect_Vertex_B33(
        float2 UV,
        float3 Tangent,
        float3 Binormal,
        float Radius,
    ...
    //BLOCK_BEGIN Line_Vertex 56

    void Line_Vertex_B56(
        float2 Scale_XY,
        float Transition,
    ...
    //BLOCK_BEGIN Blob_Vertex 43

    void Blob_Vertex_B43(
        float3 Position,
        float3 Normal,
        float3 Tangent,
        float3 Bitangent,
    ...
    //BLOCK_BEGIN Object_To_World_Dir 16

    void Object_To_World_Dir_B16(
        float3 Dir_Object,
    ...
    //BLOCK_BEGIN Edge_AA_Vertex 58

    void Edge_AA_Vertex_B58(
    ...
    VertexOutput vert(VertexInput vertInput)
    {
        ...
        Tangent_World_Q14 = (mul((float3x3)unity_ObjectToWorld, float3(1,0,0)));
        
        float3 Binormal_World_Q16;
        Object_To_World_Dir_B16(float3(0,1,0),Binormal_World_Q16);

        // Anisotropy
        float Anisotropy_Q28=length(Tangent_World_Q14)/length(Binormal_World_Q16);

        // Object_To_World_Normal
        float3 Nrm_World_Q13;
        Nrm_World_Q13 = normalize(UnityObjectToWorldNormal(vertInput.normal));
        
        // Normalize3
        float3 Tangent_World_N_Q17 = normalize(Tangent_World_Q14);

        // Normalize3
        float3 Binormal_World_N_Q18 = normalize(Binormal_World_Q16);

        float3 Pos_World_Q24;
        Object_To_World_Pos_B12(float3(0,0,0),Pos_World_Q24);

        float3 New_P_Q38;
        float2 New_UV_Q38;
        float Radial_Gradient_Q38;
        float3 Radial_Dir_Q38;
        Move_Verts_B38(Anisotropy_Q28,vertInput.vertex.xyz,_Radius_,New_P_Q38,New_UV_Q38,Radial_Gradient_Q38,Radial_Dir_Q38);

        // Incident3
        float3 Incident_Q20 = normalize(Pos_World_Q24 - _WorldSpaceCameraPos);

        float3 Pos_World_Q12;
        Object_To_World_Pos_B12(New_P_Q38,Pos_World_Q12);

        float4 Blob_Info_Q42;
        if (_Blob_Enable_) {
          Blob_Vertex_B42(Pos_World_Q12,Nrm_World_Q13,Tangent_World_N_Q17,Binormal_World_N_Q18,_Blob_Position_,_Blob_Intensity_,_Blob_Near_Size_,_Blob_Far_Size_,_Blob_Near_Distance_,_Blob_Far_Distance_,_Blob_Fade_Length_,_Blob_Pulse_,_Blob_Fade_,Blob_Info_Q42);
        } else {
          Blob_Info_Q42 = float4(0,0,0,0);
        }

        float4 Blob_Info_Q43;
        if (_Blob_Enable_2_) {
          Blob_Vertex_B43(Pos_World_Q12,Nrm_World_Q13,Tangent_World_N_Q17,Binormal_World_N_Q18,_Blob_Position_2_,_Blob_Intensity_,_Blob_Near_Size_2_,_Blob_Far_Size_,_Blob_Near_Distance_,_Blob_Far_Distance_,_Blob_Fade_Length_,_Blob_Pulse_2_,_Blob_Fade_2_,Blob_Info_Q43);
        } else {
          Blob_Info_Q43 = float4(0,0,0,0);
        }

        // DotProduct3
        float Dot_Q23;
        Dot_Q23 = dot(Tangent_World_N_Q17, Incident_Q20);
        //Dot_Q23 = sign(Dot_Q23)*Dot_Q23*Dot_Q23;
        
        float Gradient1_Q58;
        float Gradient2_Q58;
        if (_Smooth_Edges_) {
          Edge_AA_Vertex_B58(Pos_World_Q12,Nrm_World_Q13,vertInput.vertex.xyz,vertInput.normal,_WorldSpaceCameraPos,Radial_Gradient_Q38,Radial_Dir_Q38,vertInput.tangent,Gradient1_Q58,Gradient2_Q58);
        } else {
          Gradient1_Q58 = 1;
          Gradient2_Q58 = 1;
        }

        float3 Iridescence_Q40;
        Iridescence_Vertex_B40(_Iridescence_Intensity_,_Iridescence_Ramp_,_Left_X0_,_Left_X1_,_Right_X0_,_Right_X1_,_Angle_,New_UV_Q38,Dot_Q23,Iridescence_Q40);

        float2 Rect_UV_Q33;
        float4 Rect_Parms_Q33;
        float2 Scale_XY_Q33;
        Round_Rect_Vertex_B33(New_UV_Q38,Tangent_World_Q14,Binormal_World_Q16,_Radius_,0,Anisotropy_Q28,Gradient1_Q58,Gradient2_Q58,Rect_UV_Q33,Rect_Parms_Q33,Scale_XY_Q33);

        float3 Line_Vertex_1_Q56;
        float3 Line_Vertex_2_Q56;
        Line_Vertex_B56(Scale_XY_Q33,_Draw_,_Draw_Fuzz_,_Draw_Start_,Rect_UV_Q33,_Time.y,_Rate_,_Highlight_Transform_,Line_Vertex_1_Q56,Line_Vertex_2_Q56);

        float3 Position = Pos_World_Q12;
        float3 Normal = Iridescence_Q40;
        float2 UV = Rect_UV_Q33;
        float3 Tangent = Line_Vertex_1_Q56;
        float3 Binormal = Line_Vertex_2_Q56;
        float4 Color = float4(1,1,1,1);
        float4 Extra1 = Rect_Parms_Q33;
        float4 Extra2 = Blob_Info_Q42;
        float4 Extra3 = Blob_Info_Q43;
    ...
    //BLOCK_BEGIN Round_Rect_Fragment 55

    void Round_Rect_Fragment_B55(
        half Radius,
        half Line_Width,
        half4 Line_Color,
        half Filter_Width,
    ...
    //BLOCK_BEGIN Blob_Fragment 46

    void Blob_Fragment_B46(
    ...
    half4 frag(VertexOutput fragInput) : SV_Target
    {
        ...
        // Divide
        half Quotient_Q49 = 1 / _Draw_Fuzz_;

        // Add_Colors
        float4 Base_And_Iridescent_Q26;
        Base_And_Iridescent_Q26 = _Base_Color_ + float4(fragInput.normalWorld.xyz,0.0);
        
        float4 Blob_Color_Q46;
        if (_Blob_Enable_) {
          Blob_Fragment_B46(_Blob_Texture_,fragInput.extra2,Blob_Color_Q46);
        } else {
          Blob_Color_Q46 = float4(0,0,0,0);
        }

        float4 Blob_Color_Q47;
        if (_Blob_Enable_2_) {
          Blob_Fragment_B46(_Blob_Texture_,fragInput.extra3,Blob_Color_Q47);
        } else {
          Blob_Color_Q47 = float4(0,0,0,0);
        }

        half In_Line_Q48;
        float4 Line_Color_Q48;
        Line_Fragment_B48(_Line_Color_,_Highlight_Color_,_Highlight_Width_,fragInput.tangent.xyz,fragInput.binormal.xyz,Quotient_Q49,In_Line_Q48,Line_Color_Q48);

        // Combine_Blobs
        float4 Blobs_Q50;
        Blobs_Q50 = Blob_Color_Q46+Blob_Color_Q47;
        Blobs_Q50=min(Blobs_Q50,float4(1.0,1.0,1.0,1.0));
        
        // Blend_Over
        float4 Result_Q36 = Blobs_Q50 + (1.0 - Blobs_Q50.a) * Base_And_Iridescent_Q26;

        half4 Color_Q55;
        Round_Rect_Fragment_B55(_Radius_,_Line_Width_,Line_Color_Q48,_Filter_Width_,fragInput.uv,In_Line_Q48,fragInput.extra1,Result_Q36,Color_Q55);

        // Scale_Color
        float4 Result_Q52 = _Fade_Out_ * Color_Q55;

        float4 Out_Color = Result_Q52;
